...
describe("Audio Properties", () => {
  it("should handle pitch buffer operations correctly", () => {
    fc.assert(
      fc.property(fc.array(fc.float({ min: -1, max: 1 }), { minLength: 1, maxLength: 50 }), (audioBuffer) => {
    ...
    fc.assert(
      fc.property(
        ...
        (volume, buffer) => {
          ...
          const rms = validSamples.length > 0 ? Math.sqrt(validSamples.reduce((acc, val) => acc + val * val, 0) / validSamples.length) : 0;
